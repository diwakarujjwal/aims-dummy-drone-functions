...
while True:
    ...
    if results.hand_landmarks:
        for i, hand_lms in enumerate(results.hand_landmarks):
            ...
            input_data = np.array(pred_row, dtype=np.float32)
            input_data = input_data.reshape(1,-1)
            wrist_pos = (int(hand_lms[0].x * w), int(hand_lms[0].y * h))

            if is_right_hand == 1:
                right_hand_data = {"input_data": input_data, "wrist": wrist_pos, "lms": hand_lms}
            else:
                left_hand_data = {"input_data": input_data, "wrist": wrist_pos}

    final_util_cmd = "HOVER"

    if left_hand_data and right_hand_data:
        probs_util = model_util.predict(left_hand_data["input_data"], verbose=0)[0]
        if max(probs_util) > 0.6:
            final_util_cmd = UTIL_MAP[probs_util.argmax()]
    ...
    if right_hand_data:
        ...
        if fence_mode == "FOLLOW_ME":
            ...
            probs_main = model.predict(right_hand_data["input_data"], verbose=0)[0]
        ...
        probs_main = model.predict(left_hand_data["input_data"], verbose=0)[0]
        raw_main = pending_main_gesture
